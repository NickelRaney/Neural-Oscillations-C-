function [res]=model_LIF3(param,ve,vi,he,hi)
...
for step=2:duration/dt
    ...
    if spikecount_e>0
        ve(sind_e)=0;
        ref_e_t = -log(1-rand(1,spikecount_e))*ref_e;
        refc_e(sind_e)=refc_e(sind_e)+ceil(ref_e_t/dt);
    ...
    if spikecount_i>0
        vi(sind_i)=0;
        ref_i_t = -log(1-rand(1,spikecount_i))*ref_i;
        refc_i(sind_i)=refc_i(sind_i)+ceil(ref_i_t/dt);

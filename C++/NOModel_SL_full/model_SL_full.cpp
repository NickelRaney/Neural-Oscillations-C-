...
void spikeE(const int whichHit, Vector<double>& Clock, 
	vector<int>& VE, Vector<int>& HEE, Vector<int>& HIE, Vector<int>& HSE, Vector<int>& Eref,
...
void update(vector<double>& time_spike, vector<int>& num_spike, vector<double>& record_time_point, vector<int>& total_HEE,
    vector<int>& total_HIE, vector<int>& total_HSE, vector<int>& total_HEI,
    vector<int>& total_HII, vector<int>& total_HSI, vector<int>& total_HES,
    vector<int>& total_HIS, vector<int>& total_HSS, vector<int>& V_e_distribution, vector<int>& V_i_distribution, vector<int>& V_s_distribution, Vector<double>& Clock, vector<int>& VE,
...
{
    ifstream inf;
    inf.open("..//model_SL_full_params.txt");
    string s;
    getline(inf, s);
    ...
    update(time_spike, num_spike, record_time_point, total_HEE, total_HIE, total_HSE, total_HEI, total_HII, total_HSI, total_HES, total_HIS, total_HSS, V_e_distribution, V_i_distribution, V_s_distribution, Clock, VE, VI, VS,
    ...
    save_path = "..//outputs//model_SL_full";
    if (_access(save_path.c_str(), 0) == -1)
        _mkdir(save_path.c_str());

    myfile.open("..//outputs//model_SL_full//spike.txt");
    ...
    myfile.open("..//outputs//model_SL_full//H.txt");
    for (int i = 0; i < total_HIE.size(); i++)
    {
        myfile << record_time_point[i] << " " << total_HEE[i] << " " << total_HIE[i] << " " << total_HSE[i] << " " << total_HEI[i] << " " << total_HII[i] << " " << total_HSI[i] << " " << total_HES[i] << " " << total_HIS[i] << " " << total_HSS[i] << endl;
    }
    myfile.close();
    myfile.open("..//outputs//model_SL_full//V.txt");
    for (int i = 0; i < total_HIE.size(); i++)
    {
        for (int j = 0; j < NE; j++)
        {
